...
def get_gamer():
    ...
    results = dbh.run_statement(
        'CALL get_gamer(?)', [request.headers.get('id')])
...
def gamer_signup():
    ...
    token = uuid4().hex
    results = dbh.run_statement('CALL gamer_signup(?,?,?)', [
...
def delete_gamer():
    ...
    results = dbh.run_statement('CALL gamer_delete(?)', [
...
def logout_gamer():
    ...
    results = dbh.run_statement('CALL gamer_logout(?)', [
...
def update_points():
    ...
    is_valid = apih.check_endpoint_info(request.json, ['points'])
    if (is_valid != None):
        return make_response(json.dumps(is_valid, default=str), 400)

    results = dbh.run_statement('CALL points_update(?,?)', [
                                request.json.get('points'), request.headers.get('gamer_id')])
    if (type(results) == list):
        return make_response(json.dumps(results, default=str), 200)
    else:
        return make_response(json.dumps('sorry, error', default=str), 500)
